...
import com.chuangwei.internalAnswer.model.Message;
import com.chuangwei.internalAnswer.model.User;
import com.chuangwei.internalAnswer.model.ViewObject;
import com.chuangwei.internalAnswer.service.MessageService;
...
public class MessageController {
    ...
    public String addMessage(@RequestParam("toName") String toName,
        ...
        try {
            if (hostHolder.getUser() == null) {
                return InternalAnswerUtil.getJSONString(999, "未登录");
            }
            User user = userService.selectByName(toName);
            if (user == null) {
                return InternalAnswerUtil.getJSONString(1, "用户不存在");
            }
            ...
            return InternalAnswerUtil.getJSONString(1, "插入站内信失败");
        }
    }
    ...
    public String addMessage(@RequestParam("fromId") int fromId,
        ...
        try {
            ...
            //msg.setConversationId(fromId < toId ? String.format("%d_%d", fromId, toId) : String.format("%d_%d", toId, fromId));
            messageService.addMessage(msg);
            return InternalAnswerUtil.getJSONString(msg.getId());
        } catch (Exception e) {
            logger.error("增加评论失败" + e.getMessage());
            return InternalAnswerUtil.getJSONString(1, "插入评论失败");
        }
    }
}
